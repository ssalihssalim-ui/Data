...
// ============================================================
import { auth, db, showToast, showPage } from './script.js';
...
const SECTIONS = {
    ...
    produits: {
        ...
        fields: [
            ...
            { name: 'description', label: 'Description', type: 'text', required: false },
            { name: 'image', label: 'Emoji image', type: 'text', required: false }
        ],
...
window.openCrudModal = function(section, id = null) {
    ...
    crudFields.innerHTML = config.fields.map(f => `
        <div class="form-group">
            ...
            ` : `
                <input type="${f.type}" id="crud_${f.name}" ${f.required ? 'required' : ''} />
            `}
        </div>
    `).join('');
...
// STATISTIQUES
// ============================================================
// Override pour la section statistiques
...
loadSection = async function(section) {
    ...
    // Dépenses et options (pages simples)
    if (section === 'depenses') {
        const content = document.getElementById('dashboardSectionContent');
        const title = document.getElementById('dashboardTitle');
        const subtitle = document.getElementById('dashboardSubtitle');
    ...
    // Appeler la fonction originale pour les autres sections
...
console.log('📊 Admin.js chargé - CRUD complet avec Firestore');
